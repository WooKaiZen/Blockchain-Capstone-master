...
contract ERC165 {
    ...
    /**
     * contract implementing SupportsInterfaceWithLookup
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(to!=curOwner,"given address is the owner of the token");
        require((msg.sender==curOwner)||isApprovedForAll(curOwner,msg.sender),"caller not approved to transfer the token");
        _tokenApprovals[tokenId]=to;
        emit Approval(curOwner,to,tokenId);
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        require(from==_tokenOwner[tokenId],"first address provided is not the owner of the token");
    ...
    {
        if (msg.sender == tx.origin) { // not a contract
            return true;
        }
...
contract ERC721Metadata is ERC721Enumerable { //, usingOraclize 
    ...
    // = should be the _baseTokenURI + the tokenId in string form
    // see https://github.com/oraclize/ethereum-api/blob/master/oraclizeAPI_0.5.sol for strConcat()
    // you can also use uint2str() to convert a uint to a string
